<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <h1 class="text-2xl font-bold mb-4">
            @lang('site.edit') @lang('site.daily_purchase')
        </h1>
        ...
        <form action="{{ route('dashboard.daily_purchases.update', $dailyPurchase->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="tax_rate" value="{{ old('tax_rate', $dailyPurchase->tax_rate) }}" class="w-full border border-gray-300 rounded p-2" min="0" max="100">
                @error('tax_rate')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
