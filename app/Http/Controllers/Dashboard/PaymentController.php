...
class PaymentController extends Controller
{
    ...
    {
        $expenses = DailySale::where('status', 'confirmed')
            ->selectRaw('
    ...
    {
        $revenues = DailyPurchase::where('status', 'confirmed')
            ->selectRaw('
    ...
    {
        // Sales query (raw total, only confirmed)
        $sales = DailySale::where('status', 'confirmed')
            ->selectRaw('
                YEAR(sale_date) as year,
                MONTH(sale_date) as month,
        ...
        // Expenses query (raw total, only confirmed)
        $expenses = DailyPurchase::where('status', 'confirmed')
            ->selectRaw('
                YEAR(purchase_date) as year,
                MONTH(purchase_date) as month,
